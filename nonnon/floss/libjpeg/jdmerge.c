...
#define jdmerge_FIX(x)		((INT32) ((x) * (1L<<SCALEBITS) + 0.5))
...
jdmerge_build_ycc_rgb_table (j_decompress_ptr cinfo)
{
  ...
  for (i = 0, x = -CENTERJSAMPLE; i <= MAXJSAMPLE; i++, x++) {
    ...
    upsample->Cr_r_tab[i] = (int)
		    RIGHT_SHIFT(jdmerge_FIX(1.40200) * x + ONE_HALF, SCALEBITS);
    /* Cb=>B value is nearest int to 1.77200 * x */
    upsample->Cb_b_tab[i] = (int)
		    RIGHT_SHIFT(jdmerge_FIX(1.77200) * x + ONE_HALF, SCALEBITS);
    /* Cr=>G value is scaled-up -0.71414 * x */
    upsample->Cr_g_tab[i] = (- jdmerge_FIX(0.71414)) * x;
    /* Cb=>G value is scaled-up -0.34414 * x */
    /* We also add in ONE_HALF so that need not do it in inner loop */
    upsample->Cb_g_tab[i] = (- jdmerge_FIX(0.34414)) * x + ONE_HALF;
  }
}
